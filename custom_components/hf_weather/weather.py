...
class HeFengWeather(WeatherEntity):
    ...
    def condition(self):
        ...
        if self._condition:
            ks = [k for k, v in CONDITION_CLASSES.items() if self._condition in v]
            if len(ks) > 0:
                return ks[0]
            else:
                _LOGGER.error("cannot find condition class.condition=" + self._condition)
        else:
            return 'unknown'
    ...
    def device_state_attributes(self):
        ...
        if self._condition is not None:
            return {
                ...
                ATTR_ICON: self.weather_icon(),
                ATTR_CUSTOM_UI_MORE_INFO: "hf_weather-more-info"
            }

    def weather_icon(self):
        return '/local/custom-lovelace/hf_weather-card/icons/animated/' + self.state + '.svg'
...
class WeatherData(object):
    ...
    def async_update(self, now):
        ...
        self._suggestion = [
            {'title': k, 'title_cn': TRANSLATE_SUGGESTION.get(k, k), 'brf': v.get('brf'), 'txt': v.get('txt')} for k, v
            in all_result["suggestion"].items()]
        ...
        self._daily_forecast = [
            [forec_cond[0], int(datemsg[0]["tmp"]["max"]), int(datemsg[0]["tmp"]["min"]), datemsg[0]["date"],
             datemsg[0]["pcpn"], datemsg[0]["pop"]],
            [forec_cond[1], int(datemsg[1]["tmp"]["max"]), int(datemsg[1]["tmp"]["min"]), datemsg[1]["date"],
             datemsg[1]["pcpn"], datemsg[1]["pop"]],
            [forec_cond[2], int(datemsg[2]["tmp"]["max"]), int(datemsg[2]["tmp"]["min"]), datemsg[2]["date"],
             datemsg[2]["pcpn"], datemsg[2]["pop"]],
            [forec_cond[3], int(datemsg[3]["tmp"]["max"]), int(datemsg[3]["tmp"]["min"]), datemsg[3]["date"],
             datemsg[3]["pcpn"], datemsg[3]["pop"]],
            [forec_cond[4], int(datemsg[4]["tmp"]["max"]), int(datemsg[4]["tmp"]["min"]), datemsg[4]["date"],
             datemsg[4]["pcpn"], datemsg[4]["pop"]],
            [forec_cond[5], int(datemsg[5]["tmp"]["max"]), int(datemsg[5]["tmp"]["min"]), datemsg[5]["date"],
             datemsg[5]["pcpn"], datemsg[5]["pop"]],
            [forec_cond[6], int(datemsg[6]["tmp"]["max"]), int(datemsg[6]["tmp"]["min"]), datemsg[6]["date"],
             datemsg[6]["pcpn"], datemsg[6]["pop"]]
        ]
